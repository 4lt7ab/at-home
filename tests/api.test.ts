...
function json(path: string, init?: RequestInit) {
  return app.request(path, init).then(async (r) => ({
    status: r.status,
...
beforeAll(async () => {
  ...
  // Wire up error handling to match production server
  app.onError((err, c) => {
...
describe("GET /api/notes", () => {
  test("returns paginated list", async () => {
    ...
    expect(body.total).toBeGreaterThanOrEqual(4); // 1 + 3 from batch
    expect(body.data[0]).toHaveProperty("has_context");
    expect(body.data[0]).not.toHaveProperty("context"); // summaries, not full entities
  });
  ...
  test("defaults limit to 50, clamps to 200", async () => {
    // We can't easily test the upper bound without 200+ notes,
    // but we can verify a nonsense limit falls back gracefully
...
describe("PATCH /api/notes", () => {
  ...
  test("updates title", async () => {
    ...
    expect(body[0].context).toBe("original"); // untouched
  });
